...
Public Class Worker
    ...
    Public textfiletoparse As String = ""
    ...
    Private renameterms As ArrayList
...
#End Region
    ...
    Public Sub WorkerExecute()
        Try
            renameterms.Clear()
            Dim filereader As StreamReader = New StreamReader(textfiletoparse)
            Dim lineread As String
            While filereader.Peek > -1
                lineread = filereader.ReadLine.Trim
                If (Not lineread = "") And (Not lineread = Nothing) Then
                    renameterms.Add(lineread)
                End If
            ...
            Try
                If FolderWalkChosen = True Then
                    ...
                    dir = Nothing
                Else
                    Dim fir As FileInfo = New FileInfo(textfiletoparse)
                    If fir.Exists = True Then
                        'TextFileWalker(textfiletoparse)
                    End If
                    fir = Nothing
                End If
            Catch ex As Exception
                Error_Handler(ex, "Renaming Files")
            End Try
        ...
        Try
            ...
            For Each fileName In fileEntries
                Try
                    ...
                    Dim filetoreplacewith As String = ""
                    If renameterms.Count > 0 Then
                        filetoreplacewith = (finfo.DirectoryName & "\" & renameterms.Item(0).ToString()).Replace("\\", "\")
                        renameterms.RemoveAt(0)
                        filetoreplacewith = filetoreplacewith & ("." & finfo.Extension).Replace("..", ".")
                        Dim oldname As String = finfo.Name
                        Dim fmove As FileInfo = New FileInfo(filetoreplacewith)
                        If fmove.Exists = False Then
                            finfo.MoveTo(filetoreplacewith)
                        End If
